...
mod tests {
    ...
    fn test_gas_estimate_bumping() {
        ...
        let bumped_price = original_estimate.gas_price.unwrap() * U256::from(110) / U256::from(100);
    ...
    mod transaction_builder_tests {
        ...
        fn test_eip1559_transaction_request() {
            ...
            assert!(gas_estimate.max_priority_fee_per_gas.unwrap() < gas_estimate.max_fee_per_gas.unwrap());
        }
    }
}
