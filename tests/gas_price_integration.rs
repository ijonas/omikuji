use omikuji::config::models::{OmikujiConfig, Network, Datafeed, GasConfig};
use omikuji::gas_price::{GasPriceManager, models::{GasPriceFeedConfig, CoinGeckoConfig}};
use std::sync::Arc;
...
async fn test_gas_price_manager_creation() {
    ...
    assert!(cost.is_none(), "Should not calculate USD cost without prices");
}
...
async fn test_contract_updater_with_gas_price_manager() {
    ...
    assert!(gas_price_manager.is_enabled(), "Gas price manager should be enabled");
    ...
    assert!(cost.is_none(), "USD cost should be None without cached prices");
}
...
async fn test_gas_price_cache_fallback() {
    ...
    let networks = vec!["ethereum-mainnet".to_string(), "polygon-mainnet".to_string()];
    ...
    assert!(config.gas_price_feeds.fallback_to_cache, "Fallback should be enabled");
}
...
async fn test_usd_cost_metrics_recording() {
    ...
    assert!(expected_usd < 10.0, "Basic transfer should cost less than $10");
